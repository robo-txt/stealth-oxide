...
container_id="$(docker run -d --rm \
    ...
    -c 'Xvfb :99 -screen 0 1920x1080x24 -nolisten tcp >/tmp/xvfb.log 2>&1 &
        ...
        exec /usr/bin/chromium --headless=new --ozone-platform=x11 --no-sandbox \
            ...
            --user-data-dir=/tmp/chrome-cdp-profile --enable-gpu --use-gl=angle \
...
output="$artifact_dir/cdp-target-probe.json"
log="$artifact_dir/cdp-target-probe.log"
timeout --foreground -k 3s "${CASE_TIMEOUT:-60}s" \
    cargo run --offline --quiet --manifest-path \
    "$repo_root/dev/webscraping-research/cdp-target-probe/Cargo.toml" \
    >"$output" 2>"$log"
status=$?
